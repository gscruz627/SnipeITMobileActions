...
function Home(){
  ...
  async function displayLocation(){
    ...
    alert(`past resposne, checking existance, response: ${response}`);
...
App.addListener('appStateChange', async function({isActive} : {isActive:boolean}){
  // This event listener on App itself will be executed when the app changes to background running, in that case,
  // close any cameras, whenever the app starts again, start the scanning (assuming isScanning is true).
  if(!isActive){
    stopCameraCurrent();
  } else {
    stopCameraCurrent();
